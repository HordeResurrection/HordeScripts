...
import { DiplomacyStatus, UnitDirection, DrawLayer, FontUtils, GeometryVisualEffect, GeometryCanvas, Stride_Vector2, Stride_Color, UnitCommand } from "library/game-logic/horde-types";
import { unitCanBePlacedByRealMap } from "library/game-logic/unit-and-map";
import HordePluginBase from "plugins/base-plugin";
import { Cell } from "TowerProtection/Types/Geometry";
...
export class AutoFFAPlugin extends HordePluginBase {
    ...
    public constructor() {
        ...
        this._HordeEngine = HordeEngine;
        this._ActiveScena = ActiveScena;
    ...
    /** таблица мира */
    _settlements_settlements_diplomacyStatus : Array<Array<any>>;
    ...
    _settlements_castleCfg : Array<any>;
    /** главный замок поселения */
    _settlements_castle: Array<any>;
    /** рамка вокруг главного замка */
    _settlements_castleFrame: Array<any>;
    ...
    _settlements_powerPointStrDecorators : Array<any>;
    ...
    _settlements_statucStrDecorators : Array<any>;
    ...
    public onEveryTick(gameTickNum: number) {
        var FPS = this._HordeEngine.HordeResurrection.Engine.Logic.Battle.BattleController.GameTimer.CurrentFpsLimit;
    ...
    private _Init() {
        var scenaSettlements = this._ActiveScena.GetRealScena().Settlements;
        ...
        for (var player of this._Players) {
            ...
            if (settlementsUid.find((settlementUid) => { return (settlementUid == settlement.Uid); })) {
                continue;
            }
            this.log.info("Замечен игрок поселения ", settlement.Uid);
            settlementsUid.push(settlement.Uid);
        }
        settlementsUid.sort();

        // поселения

        this._settlements = new Array<any>();
        this._opSettlementUidToSettlementNum = new Array<any>(this._ActiveScena.GetRealScena().Settlements.Count).fill(-1);
        for (var settlementNum = 0; settlementNum < settlementsUid.length; settlementNum++) {
            ...
            this._opSettlementUidToSettlementNum[settlement.Uid] = this._settlements.length;
            this._settlements.push(settlement);
        }

        // объявляем мир между всеми

        this._settlements_settlements_diplomacyStatus = new Array<Array<any>>(this._settlements.length);
        for (var settlementNum = 0; settlementNum < this._settlements.length; settlementNum++) {
            this._settlements_settlements_diplomacyStatus[settlementNum] = new Array<any>(this._settlements.length);
        ...
        for (var player of this._Players) {
            ...
            var settlementNum : number = settlementsUid.findIndex((Uid) => { return Uid ==  settlement.Uid});
            if (settlementNum == -1) {
                continue;
            }
        ...
        this._settlements_castleCfg = new Array<any>();
        this._settlements_castle    = new Array<any>();
        for (var settlementNum = 0; settlementNum < this._settlements.length; settlementNum++) {
        ...
        for (var settlementNum = 0; settlementNum < this._settlements.length; settlementNum++) {
            ...
            ScriptUtils.SetValue(principalInstruction, "AlmostDefeatCondition", this._HCL.HordeClassLibrary.World.Settlements.Existence.AlmostDefeatCondition.Custom);
            ScriptUtils.SetValue(principalInstruction, "TotalDefeatCondition", this._HCL.HordeClassLibrary.World.Settlements.Existence.TotalDefeatCondition.Custom);
            ScriptUtils.SetValue(principalInstruction, "VictoryCondition", this._HCL.HordeClassLibrary.World.Settlements.Existence.VictoryCondition.Custom);
        }
        ...
        this._settlements_castleFrame = new Array<any>();
        for (var settlementNum = 0; settlementNum < this._settlements.length; settlementNum++) {
            this._settlements_castleFrame.push(null);

            let position            = this._settlements_castle[settlementNum].Position;
            let ticksToLive         = GeometryVisualEffect.InfiniteTTL;
            this._settlements_castleFrame[settlementNum] = spawnGeometry(this._ActiveScena, this._CreateCastleFrameBuffer(settlementNum), position, ticksToLive);
        }

        // инициализируем очки власти

        this._settlements_powerPoints = new Array<number>();
        this._settlements_powerPointStrDecorators = new Array<any>();
        for (var settlementNum = 0; settlementNum < this._settlements.length; settlementNum++) {
            this._settlements_powerPoints.push(1500);
            this._settlements_powerPointStrDecorators.push(null);
            ...
            this._settlements_powerPointStrDecorators[settlementNum] = strDecObj;
        }

        // создаем строку статуса

        this._settlements_statucStrDecorators = new Array<any>();
        for (var settlementNum = 0; settlementNum < this._settlements.length; settlementNum++) {
            this._settlements_statucStrDecorators.push(null);
            ...
            this._settlements_statucStrDecorators[settlementNum] = strDecObj;
        }

        // подписываемся на событие атаки для начисление очков власти
        
        this._settlements_settlements_powerPoints = new Array<Array<number>>(this._settlements.length);
        ...
        this._settlements_nextRewardTime = new Array<number>(this._settlements.length);
        for (var settlementNum = 0; settlementNum < this._settlements.length; settlementNum++) {
    ...
    private _VassalTribute(gameTickNum: number) {
        for (var settlementNum = 0; settlementNum < this._settlements.length; settlementNum++) {
            ...
            if (tribute.Gold == 0 && tribute.Metal == 0 && tribute.Lumber == 0 && tribute.FreePeople == 0) {
                continue;
            }
    ...
    private _ChoiseEnemyTeam(gameTickNum: number) {
        ...
        let rnd = this._ActiveScena.GetRealScena().Context.Randomizer;
    ...
    private _UpdatePowerPointStrDecorators(gameTickNum: number) {
        ...
        for (var settlementNum = 0; settlementNum < this._settlements.length; settlementNum++) {
            this._settlements_statucStrDecorators[settlementNum].Text = this._settlements_suzerainNum[settlementNum] == -1 ? "сюзерен" : "вассал";
        }
    }
    ...
    private _EndGameCheck(gameTickNum) {
    ...
    private _TeamAddVassal(teamNum: number, settlementNum: number, silent: boolean = false) {
        ...
        while(enumerator.MoveNext()) {
            if (enumerator.Current.Cfg.IsBuilding) {
        ...
        var spawnPosition : any = null;
        if (sumCount == 0) {
            var castlesPositions = new Array<any>();

            enumerator = this._settlements[suzerainNum].Units.GetEnumerator();
            while(enumerator.MoveNext()) {
                if (enumerator.Current.Cfg.HasMainBuildingSpecification) {
    ...
    private _CreateCastleFrameBuffer(settlementNum: number) {
        ...
        let points = this._host.newArr(Stride_Vector2, 5);
    ...
    private _SettlementSetCastle(settlementNum: number, castleUnit: any) {
        ...
        this._settlements_statucStrDecorators[settlementNum].Position = createPoint(Math.floor(32*(castleUnit.Cell.X + 2.7)), Math.floor(32*(castleUnit.Cell.Y + 3.6)));
    }
