...
export class Spell_fiery_trail extends ISpell {
    ...
    protected static _CfgUid                : string = "Hero_fiery_trail";
    protected static _AnimationsCatalogRef  : string = "#AnimCatalog_Command_fiery_trail";
    ...
    protected _OnEveryTickActivated(gameTickNum: number): boolean {
        ...
        if (this._activatedGameTick + Spell_fiery_trail._Duration <= gameTickNum) {
        ...
        this._trailCells.forEach(cell => {
            ...
            if (upperHordeUnit && upperHordeUnit.Owner.Uid == hero.hordeUnit.Owner.Uid){
                return;
            }
            
            // поджигаем клетку
            HordeClassLibrary.World.Objects.Bullets.Implementations.Fire.BaseFireBullet.MakeFire(
                hero.hordeUnit, cell.Scale(32).Add(new Cell(16, 16)).ToHordePoint(), UnitMapLayer.Main, Spell_fiery_trail._FireHordeConfig);
        });
