...
import { ACommandArgs, BaseBullet, BattleController, BulletConfig, BulletEmittingArgs, BulletState, DiplomacyStatus, ShotParams, Stride_Color, Unit, UnitCommandConfig, UnitMapLayer } from "library/game-logic/horde-types";
...
export class Spell_Homing_Fireball extends ISpell {
    ...
    private static _HomingInitializeWorker(bullet: BaseBullet, emitArgs: BulletEmittingArgs): void {
        ...
        bullet.ScriptData.endTick = BattleController.GameTimer.GameFramesCounter + 10*50;
    }

    private static _HomingProcessWorker(bullet: BaseBullet): void {
        // время жизни снаряда закончилось
        if (bullet.ScriptData.endTick < BattleController.GameTimer.GameFramesCounter) {
