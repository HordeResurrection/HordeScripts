...
export class IAttackPathChoiser {
    public choiseAttackPath(unit: any, settlements_attack_paths: Array<Array<Array<Cell>>>) : number {
        return 0;
    }
};
export class AttackPathChoiser_NearDistance extends IAttackPathChoiser {
    ...
    public choiseAttackPath(unit: any, settlements_attack_paths: Array<Array<Array<Cell>>>) : number {
        ...
        for (var attackPathNum = 0; attackPathNum < settlements_attack_paths[unit.Owner.Uid].length; attackPathNum++) {
            var distance = 0.0;
            switch (this._metricType) {
                case MetricType.L1:
                    distance = distance_L1(unit.Cell.X, unit.Cell.Y, settlements_attack_paths[unit.Owner.Uid][attackPathNum][0].X, settlements_attack_paths[unit.Owner.Uid][attackPathNum][0].Y);
                    break;
                case MetricType.L2:
                    distance = distance_L2(unit.Cell.X, unit.Cell.Y, settlements_attack_paths[unit.Owner.Uid][attackPathNum][0].X, settlements_attack_paths[unit.Owner.Uid][attackPathNum][0].Y);
                    break;
            }
...
export class AttackPathChoiser_Periodically extends IAttackPathChoiser {
    nextAttackPathNum: Array<number>;

    public constructor() {
        super();

        this.nextAttackPathNum = [];
    }

    public choiseAttackPath(unit: any, settlements_attack_paths: Array<Array<Array<Cell>>>) : number {
        if (this.nextAttackPathNum.length != settlements_attack_paths.length) {
            this.nextAttackPathNum = new Array<number>(settlements_attack_paths.length);
            for (var settlementId = 0; settlementId < settlements_attack_paths.length; settlementId++) {
                this.nextAttackPathNum[settlementId] = 0;
            }
        }

        this.nextAttackPathNum[unit.Owner.Uid] = (this.nextAttackPathNum[unit.Owner.Uid] + 1) % settlements_attack_paths[unit.Owner.Uid].length;
        return this.nextAttackPathNum[unit.Owner.Uid];
    }
};
